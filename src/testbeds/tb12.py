"""TB-12 adversarial / Byzantine robustness evaluation."""

from __future__ import annotations

import math
import random
from dataclasses import dataclass
from pathlib import Path
from typing import Dict, List, Tuple

import numpy as np
import torch
import yaml
from torch import nn
from torch.utils.data import DataLoader, TensorDataset

from src.data.cmapss import prepare_cmapss_dataset
from src.data.partitioning import dirichlet_quantity_skew
from src.testbeds.tb04 import (
    DatasetConfig as TB04DatasetConfig,
    ClientConfig as TB04ClientConfig,
    FederatedConfig as TB04FederatedConfig,
    ModelConfig as TB04ModelConfig,
    SequenceRegressor,
    ClientPartition as TB04ClientPartition,
    _apply_feature_shift,
    _feature_columns,
    _windowed_sequences,
)


@dataclass(slots=True)
class EvalConfig:
    device: str
    seed: int
    max_relative_degradation: float
    detection_zscore: float


@dataclass(slots=True)
class AttackScenario:
    name: str
    attack_type: str
    malicious_fraction: float
    severity: float
    baseline: bool = False


@dataclass(slots=True)
class TB12Config:
    dataset: TB04DatasetConfig
    clients: TB04ClientConfig
    federated: TB04FederatedConfig
    model: TB04ModelConfig
    evaluation: EvalConfig
    attack_scenarios: List[AttackScenario]


def load_tb12_config(config_path: Path) -> TB12Config:
    data = yaml.safe_load(config_path.read_text(encoding="utf-8"))

    dataset_cfg = TB04DatasetConfig(
        dataset_id=data["dataset"].get("id", "FD002"),
        sequence_length=int(data["dataset"].get("sequence_length", 30)),
        sequence_stride=int(data["dataset"].get("sequence_stride", 10)),
        normalize=bool(data["dataset"].get("normalize", True)),
        max_sequences_per_client=int(data["dataset"].get("max_sequences_per_client", 400)),
    )
    clients_cfg = TB04ClientConfig(
        num_clients=int(data["clients"].get("num_clients", 20)),
        dirichlet_alpha=float(data["clients"].get("dirichlet_alpha", 0.6)),
        feature_shift_std=float(data["clients"].get("feature_shift_std", 0.05)),
        seed=int(data["clients"].get("seed", 2035)),
    )
    federated_cfg = TB04FederatedConfig(
        rounds=int(data["federated"].get("rounds", 100)),
        clients_per_round=float(data["federated"].get("clients_per_round", 1.0)),
        local_epochs=int(data["federated"].get("local_epochs", 1)),
        batch_size=int(data["federated"].get("batch_size", 32)),
        test_batch_size=int(data["federated"].get("test_batch_size", 128)),
        lr=float(data["federated"].get("optimizer", {}).get("lr", 1e-3)),
        weight_decay=float(data["federated"].get("optimizer", {}).get("weight_decay", 0.0)),
        fedprox_mu=float(data["federated"].get("fedprox_mu", 0.01)),
        scaffold_eta=float(data["federated"].get("scaffold_eta", 1.0)),
    )
    model_cfg = TB04ModelConfig(
        hidden_size=int(data["model"].get("hidden_size", 64)),
        num_layers=int(data["model"].get("num_layers", 1)),
        dropout=float(data["model"].get("dropout", 0.2)),
    )
    eval_cfg = EvalConfig(
        device=str(data["evaluation"].get("device", "cpu")),
        seed=int(data["evaluation"].get("seed", 2035)),
        max_relative_degradation=float(data["evaluation"].get("max_relative_degradation", 0.1)),
        detection_zscore=float(data["evaluation"].get("detection_zscore", 2.5)),
    )

    scenarios: List[AttackScenario] = []
    for entry in data.get("attack_scenarios", []):
        scenarios.append(
            AttackScenario(
                name=str(entry.get("name", "scenario")),
                attack_type=str(entry.get("attack_type", "none")),
                malicious_fraction=float(entry.get("malicious_fraction", 0.0)),
                severity=float(entry.get("severity", 0.0)),
                baseline=bool(entry.get("baseline", False)),
            )
        )
    if not scenarios:
        scenarios.append(
            AttackScenario(
                name="clean_baseline",
                attack_type="none",
                malicious_fraction=0.0,
                severity=0.0,
                baseline=True,
            )
        )

    return TB12Config(
        dataset=dataset_cfg,
        clients=clients_cfg,
        federated=federated_cfg,
        model=model_cfg,
        evaluation=eval_cfg,
        attack_scenarios=scenarios,
    )


def run_tb12_pipeline(
    project_root: Path,
    algo: str,
    rounds_override: int | None = None,
) -> Tuple[List[Dict[str, float]], Dict[str, object]]:
    """Execute TB-12 workflow for a specific algorithm."""

    config_path = project_root / "config" / "config_v2" / "tb12_adversarial_robustness.yaml"
    tb12_config = load_tb12_config(config_path)
    if rounds_override is not None and rounds_override > 0:
        tb12_config.federated.rounds = int(rounds_override)

    random.seed(tb12_config.clients.seed)
    np.random.seed(tb12_config.clients.seed)
    torch.manual_seed(tb12_config.evaluation.seed)

    train_df, test_df, _ = prepare_cmapss_dataset(
        dataset_id=tb12_config.dataset.dataset_id,
        root=project_root / "data" / "c-mapss",
        normalize=tb12_config.dataset.normalize,
        rul_clip=125,
    )
    feature_cols = _feature_columns(train_df)

    partitions = dirichlet_quantity_skew(
        train_df,
        num_clients=tb12_config.clients.num_clients,
        alpha=tb12_config.clients.dirichlet_alpha,
        seed=tb12_config.clients.seed,
    )

    client_partitions: Dict[int, TB04ClientPartition] = {}
    for client_id, df in partitions.items():
        client_df = df.copy()
        if tb12_config.clients.feature_shift_std > 0:
            _apply_feature_shift(client_df, feature_cols, tb12_config.clients.feature_shift_std)
        sequences, labels = _windowed_sequences(
            client_df,
            feature_cols,
            tb12_config.dataset.sequence_length,
            tb12_config.dataset.sequence_stride,
            tb12_config.dataset.max_sequences_per_client,
        )
        if len(sequences) == 0:
            continue
        dataset = TensorDataset(
            torch.from_numpy(sequences),
            torch.from_numpy(labels),
        )
        client_partitions[client_id] = TB04ClientPartition(dataset=dataset, num_samples=len(dataset))

    if not client_partitions:
        raise RuntimeError("No client partitions available for TB-12.")

    test_sequences, test_labels = _windowed_sequences(
        test_df,
        feature_cols,
        tb12_config.dataset.sequence_length,
        tb12_config.dataset.sequence_stride,
        max_sequences=None,
    )
    test_dataset = TensorDataset(
        torch.from_numpy(test_sequences),
        torch.from_numpy(test_labels),
    )
    test_loader = DataLoader(
        test_dataset,
        batch_size=tb12_config.federated.test_batch_size,
        shuffle=False,
    )

    trainer = AttackRobustnessTrainer(
        config=tb12_config,
        clients=client_partitions,
        test_loader=test_loader,
        input_size=len(feature_cols),
    )

    scenario_results: List[Dict[str, object]] = []
    baseline_rmse = None

    for scenario in tb12_config.attack_scenarios:
        stats = trainer.run(algo, scenario)
        if scenario.baseline or scenario.malicious_fraction == 0:
            baseline_rmse = stats["rmse"]
        scenario_results.append(
            {
                "scenario": scenario.name,
                "attack_type": scenario.attack_type,
                "malicious_fraction": scenario.malicious_fraction,
                "rmse": stats["rmse"],
                "relative_degradation": None,  # filled later
                "resistant": False,
                "detection_recall": stats["detection_recall"],
                "detection_precision": stats["detection_precision"],
            }
        )

    if baseline_rmse is None and scenario_results:
        baseline_rmse = scenario_results[0]["rmse"]  # type: ignore[assignment]

    resistant_count = 0
    scenario_count = 0
    tolerated_fractions: List[float] = []
    detection_recalls: List[float] = []

    for entry in scenario_results:
        fraction = float(entry["malicious_fraction"])
        rmse = float(entry["rmse"])
        if baseline_rmse and baseline_rmse > 0:
            relative = (rmse - baseline_rmse) / baseline_rmse
        else:
            relative = 0.0
        entry["relative_degradation"] = relative

        if fraction > 0:
            scenario_count += 1
            resistant = relative <= tb12_config.evaluation.max_relative_degradation
            entry["resistant"] = resistant
            if resistant:
                resistant_count += 1
                tolerated_fractions.append(fraction)
        else:
            entry["resistant"] = True

        detection_recalls.append(float(entry["detection_recall"]))

    attack_resistance = resistant_count / scenario_count if scenario_count > 0 else 1.0
    byz_tolerance = max(tolerated_fractions) if tolerated_fractions else 0.0
    attack_detection = float(np.mean(detection_recalls)) if detection_recalls else 0.0

    measurement_entries = [
        {
            "metric_id": "M.FL.ATTACK_RES",
            "value": attack_resistance,
            "extras": {
                "resistant_scenarios": resistant_count,
                "total_scenarios": scenario_count,
                "max_relative_degradation": tb12_config.evaluation.max_relative_degradation,
            },
        },
        {
            "metric_id": "M.FL.BYZ_TOL",
            "value": byz_tolerance,
            "extras": {
                "tolerated_fraction": byz_tolerance,
                "max_relative_degradation": tb12_config.evaluation.max_relative_degradation,
            },
        },
        {
            "metric_id": "M.FL.ATTACK_DET",
            "value": attack_detection,
            "extras": {
                "average_detection_recall": attack_detection,
                "scenarios": len(detection_recalls),
            },
        },
    ]

    details = {
        "baseline_rmse": baseline_rmse,
        "scenarios": scenario_results,
        "attack_resistance": attack_resistance,
        "byzantine_tolerance": byz_tolerance,
        "attack_detection": attack_detection,
    }

    return measurement_entries, details


class AttackRobustnessTrainer:
    """Federated trainer with adversarial client simulation."""

    def __init__(
        self,
        config: TB12Config,
        clients: Dict[int, TB04ClientPartition],
        test_loader: DataLoader,
        input_size: int,
    ) -> None:
        self.config = config
        self.clients = clients
        self.test_loader = test_loader
        self.input_size = input_size
        self.device = torch.device(config.evaluation.device)
        self.criterion = nn.MSELoss()

    def run(self, algo: str, scenario: AttackScenario) -> Dict[str, float]:
        algo_key = algo.lower()
        if algo_key not in {"fedavg", "fedprox", "scaffold"}:
            raise ValueError(f"Unsupported algorithm '{algo}'.")

        global_model = self._new_model()
        global_state = global_model.state_dict()

        c_global = None
        c_clients: Dict[int, Dict[str, torch.Tensor]] = {}
        if algo_key == "scaffold":
            c_global = _zero_like_state(global_state, device=self.device)
            c_clients = {
                client_id: _zero_like_state(global_state, device=self.device)
                for client_id in self.clients
            }

        client_ids = list(self.clients.keys())
        malicious_count = max(0, int(round(scenario.malicious_fraction * len(client_ids))))
        rng = random.Random((self.config.clients.seed, scenario.name))
        malicious_clients = set(rng.sample(client_ids, malicious_count)) if malicious_count else set()

        rounds = self.config.federated.rounds
        clients_per_round = max(
            1, int(math.ceil(self.config.federated.clients_per_round * len(client_ids)))
        )

        detection_tp = 0
        detection_fp = 0
        detection_fn = 0

        for _ in range(rounds):
            random.shuffle(client_ids)
            selected = client_ids[:clients_per_round]
            updates: List[Tuple[Dict[str, torch.Tensor], int]] = []
            delta_cs: List[Dict[str, torch.Tensor]] = []
            update_norms: List[Tuple[int, float]] = []

            for client_id in selected:
                partition = self.clients[client_id]
                if partition.num_samples == 0:
                    continue
                loader = DataLoader(
                    partition.dataset,
                    batch_size=self.config.federated.batch_size,
                    shuffle=True,
                )
                local_model = self._new_model()
                local_model.load_state_dict(global_state)
                local_model.to(self.device)
                global_params = {key: value.to(self.device) for key, value in global_state.items()}

                is_malicious = client_id in malicious_clients
                update_state, sample_count, delta_c = self._train_client(
                    local_model,
                    loader,
                    algo_key,
                    global_params,
                    c_global,
                    c_clients.get(client_id),
                    scenario,
                    is_malicious,
                )
                updates.append((update_state, sample_count))
                if algo_key == "scaffold" and delta_c is not None and c_global is not None:
                    c_clients[client_id] = delta_c
                    delta_cs.append(delta_c)

                update_norm = self._update_norm(update_state, global_state)
                update_norms.append((client_id, update_norm))

            if updates:
                global_state = _aggregate_weighted(updates)
                if algo_key == "scaffold" and delta_cs and c_global is not None:
                    c_global = _update_control_variate(c_global, delta_cs, self.config.federated.scaffold_eta)

            if update_norms:
                norms = np.array([norm for _, norm in update_norms], dtype=np.float64)
                median = float(np.median(norms))
                mad = float(np.median(np.abs(norms - median))) + 1e-6
                threshold = self.config.evaluation.detection_zscore * mad
                for client_id, norm in update_norms:
                    suspicious = abs(norm - median) > threshold
                    if suspicious and client_id in malicious_clients:
                        detection_tp += 1
                    elif suspicious and client_id not in malicious_clients:
                        detection_fp += 1
                    elif (not suspicious) and client_id in malicious_clients:
                        detection_fn += 1

        global_model.load_state_dict(global_state)
        global_model.to(self.device)
        metrics = self._evaluate(global_model)

        tp = detection_tp
        fp = detection_fp
        fn = detection_fn
        recall = tp / (tp + fn) if (tp + fn) > 0 else 0.0
        precision = tp / (tp + fp) if (tp + fp) > 0 else 0.0

        return {"rmse": metrics["rmse"], "detection_recall": recall, "detection_precision": precision}

    def _train_client(
        self,
        model: SequenceRegressor,
        loader: DataLoader,
        algo: str,
        global_state: Dict[str, torch.Tensor],
        c_global: Dict[str, torch.Tensor] | None,
        c_client: Dict[str, torch.Tensor] | None,
        scenario: AttackScenario,
        is_malicious: bool,
    ) -> Tuple[Dict[str, torch.Tensor], int, Dict[str, torch.Tensor] | None]:
        model.train()
        optimizer = torch.optim.Adam(
            model.parameters(),
            lr=self.config.federated.lr,
            weight_decay=self.config.federated.weight_decay,
        )

        total_samples = 0
        total_steps = 0
        for _ in range(self.config.federated.local_epochs):
            for batch_x, batch_y in loader:
                batch_x = batch_x.to(self.device)
                batch_y = batch_y.to(self.device)

                if is_malicious:
                    batch_x, batch_y = self._apply_attack_to_batch(batch_x, batch_y, scenario)

                preds = model(batch_x)
                loss = self.criterion(preds, batch_y)
                if algo == "fedprox":
                    prox = 0.0
                    for name, param in model.named_parameters():
                        prox += torch.sum((param - global_state[name]) ** 2)
                    loss = loss + (self.config.federated.fedprox_mu / 2.0) * prox

                optimizer.zero_grad()
                loss.backward()

                if algo == "scaffold" and c_global is not None and c_client is not None:
                    for name, param in model.named_parameters():
                        param.grad = param.grad + (c_client[name] - c_global[name])

                optimizer.step()
                total_samples += batch_x.size(0)
                total_steps += 1

        state_dict = {name: tensor.detach().cpu() for name, tensor in model.state_dict().items()}

        if is_malicious and scenario.attack_type == "grad_poison":
            for name, tensor in state_dict.items():
                noise = scenario.severity * torch.randn_like(tensor)
                state_dict[name] = tensor + noise

        delta_c: Dict[str, torch.Tensor] | None = None
        if algo == "scaffold" and c_client is not None and total_steps > 0:
            delta_c = {}
            lr = self.config.federated.lr
            for name, param in state_dict.items():
                correction = (global_state[name].cpu() - param) / (lr * total_steps)
                delta_c[name] = c_client[name].cpu() - c_global[name].cpu() + correction

        return state_dict, total_samples, delta_c

    def _apply_attack_to_batch(
        self,
        batch_x: torch.Tensor,
        batch_y: torch.Tensor,
        scenario: AttackScenario,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        if scenario.attack_type == "label_flip":
            perturbation = scenario.severity * torch.sign(batch_y + 1e-6) * 50.0
            batch_y = batch_y + perturbation
        elif scenario.attack_type == "backdoor":
            batch_x[:, :, -1] = batch_x[:, :, -1] + scenario.severity * 5.0
            batch_y = batch_y - scenario.severity * 30.0
        return batch_x, batch_y

    def _update_norm(self, update_state: Dict[str, torch.Tensor], global_state: Dict[str, torch.Tensor]) -> float:
        total = 0.0
        for name in update_state.keys():
            delta = update_state[name] - global_state[name]
            total += torch.sum(delta ** 2).item()
        return math.sqrt(total)

    def _evaluate(self, model: SequenceRegressor) -> Dict[str, float]:
        model.eval()
        preds: List[float] = []
        targets: List[float] = []

        with torch.no_grad():
            for batch_x, batch_y in self.test_loader:
                batch_x = batch_x.to(self.device)
                outputs = model(batch_x).cpu().numpy()
                preds.extend(outputs.tolist())
                targets.extend(batch_y.numpy().tolist())

        preds_arr = np.array(preds, dtype=np.float64)
        targets_arr = np.array(targets, dtype=np.float64)
        rmse = float(np.sqrt(np.mean(np.square(preds_arr - targets_arr))))
        return {"rmse": rmse}

    def _new_model(self) -> SequenceRegressor:
        return SequenceRegressor(self.input_size, self.config.model)


def _zero_like_state(state: Dict[str, torch.Tensor], device: torch.device) -> Dict[str, torch.Tensor]:
    return {key: torch.zeros_like(value, device=device) for key, value in state.items()}


def _aggregate_weighted(updates: List[Tuple[Dict[str, torch.Tensor], int]]) -> Dict[str, torch.Tensor]:
    total = sum(weight for _, weight in updates)
    if total == 0:
        raise ValueError("Cannot aggregate zero-weight updates.")
    keys = updates[0][0].keys()
    aggregated: Dict[str, torch.Tensor] = {}
    for key in keys:
        num = sum(state[key] * weight for state, weight in updates)
        aggregated[key] = num / total
    return aggregated


def _update_control_variate(
    c_global: Dict[str, torch.Tensor],
    deltas: List[Dict[str, torch.Tensor]],
    eta: float,
) -> Dict[str, torch.Tensor]:
    if not deltas:
        return c_global
    avg_delta: Dict[str, torch.Tensor] = {}
    for key in c_global.keys():
        stacked = torch.stack([delta[key] for delta in deltas], dim=0)
        avg_delta[key] = stacked.mean(dim=0)
    return {key: c_global[key] + eta * avg_delta[key] for key in c_global.keys()}


